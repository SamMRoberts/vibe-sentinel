...
- Keep service traits as the only route from the application core to storage, clocks, processes, MCP transport, and other external systems.
- Keep adapters responsible for concrete I/O, serialization, and integration details, not feature policy.
- Keep CLI, ratatui TUI, and MCP surfaces as thin entry points that call the application core.
...
- Translate raw CLI flags, TUI events, and MCP payloads into explicit domain inputs before invoking core behavior.
...
- Do not let MCP request envelopes or resource payload shapes drive internal type design.
...
- If the same validation rule appears in CLI, TUI, and MCP code, move it into the core or a shared boundary utility.
